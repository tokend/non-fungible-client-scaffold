    ...
    <div class="portfolio-widget__wrapper" v-if="currentAsset">
      <div class="portfolio-widget__select">
        <div class="portfolio-widget__select-picture">
          <router-link :to="{name: 'arts.art-details', params: { id: currentAsset }}">
        ...
        <div class="portfolio-widget__select-field">
          ...
          />
          <div>
            {{ 'lbl_art_balance' | translate}}: {{ balance }}%
          </div>
        </div>
      </div>
    </div>
    ...
    computed: {
      ...
      tokensList () {
        const tokens = this.tokens.filter(token => Object.keys(this.accountBalances).includes(token.code) &&
          +this.accountBalances[token.code].balance > 0)
        ...
        const otherAssets = tokens.filter(token => !token.policies.includes(ASSET_POLICIES.baseAsset))
                                  .filter(token => !config.HIDDEN_ARTS.includes(token.code))
                                  .sort((a, b) => a.code.localeCompare(b.code))
        return [...otherAssets].map(item => `${item.name} (${item.code})`)
      },
      ...
      imgUrl () {
        const logoKey = get(this.accountBalances, `${this.currentAsset}.asset_details.details.photo.key`)
        if (logoKey) {
