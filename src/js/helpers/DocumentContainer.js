...
export class DocumentContainer {
   ...
   */
  constructor (opts) {
    if (opts.file) this.readFile(opts.file)
  ...
  async readFile (file) {
    const fileString = await FileHelper.readFileAsText(file)
    let hash = md5.create()
...
/**
 * Wraps object with doc details view {@link DocumentContainer.getDetailsForSave} to {@link DocumentContainer} instance
 * @param {object} documents - list of documents where key is document type {@link documentTypes}, value - details
 * generated by {@link DocumentContainer.getDetailsForSave}
